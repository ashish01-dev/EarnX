...
export function Navbar() {
  ...
  const logo = (
    <a
      ...
      className="font-display font-bold tracking-tight cursor-pointer text-gradient-primary"
  ...
  const navLinks = links.map((link) => {
    ...
    return (
      <a
        ...
        onMouseLeave={(e) => { (e.target as HTMLElement).style.color = active ? "#ffb780" : "#a7a9be"; }}
      >
        {link.label}
      </a>
    );
  ...
  return (
    <>
      ...
      <motion.nav
        ...
        style={{
          minWidth: 820, justifyContent: "space-between",
      ...
      >
        {logo}
        <div className="flex items-center gap-7">{navLinks}</div>
        <motion.button
      ...
      <motion.nav
        ...
        style={{
          background: "rgba(32,30,40,0.75)", backdropFilter: "blur(20px)",
      ...
      >
        <a
          ...
          className="font-display font-bold text-gradient-primary cursor-pointer"
          style={{ fontSize: 22 }}
        >
          EarnX
        </a>
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            style={{
              background: "rgba(22,22,26,0.95)", backdropFilter: "blur(40px)",
              border: "1px solid rgba(255,255,255,0.10)",
            }}
          >
            {links.map((link) => (
              <a
                ...
                style={{ color: isActive(link) ? "#ffb780" : "#a7a9be", fontFamily: "'Plus Jakarta Sans', sans-serif" }}
                onMouseEnter={(e) => { (e.target as HTMLElement).style.color = "#ffb780"; }}
